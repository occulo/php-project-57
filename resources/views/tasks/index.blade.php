    ...
    <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          {{ html()->button(__('app.buttons.common.apply'))->type('submit')->class('inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150') }}
      ...
      <table class="min-w-full table-fixed" data-test="tasks">
        <thead class="bg-gray-100 dark:bg-gray-900">
          <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
            <th class="w-1 px-6 py-4 font-medium text-center text-gray-700 dark:text-gray-300">{{ __('app.fields.id') }}</th>
            <th class="min-w-48 px-6 py-4 font-medium text-center text-gray-700 dark:text-gray-300">{{ __('app.fields.title') }}</th>
            <th class="w-36 px-6 py-4 font-medium text-center text-gray-700 dark:text-gray-300">{{ __('app.fields.status') }}</th>
            <th class="w-32 px-6 py-4 font-medium text-center text-gray-700 dark:text-gray-300">{{ __('app.fields.labels') }}</th>
            <th class="w-40 px-6 py-4 font-medium text-center text-gray-700 dark:text-gray-300">{{ __('app.fields.assigned_to_id') }}</th>
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
          @foreach($tasks as $task) <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
            ...
            <td class="px-6 py-4 text-left whitespace-normal break-words text-gray-900 dark:text-gray-100">{{ $task->assignedTo?->name ?? __('app.fields.empty.assignee') }}</td>
            <td class="px-6 py-4 text-left whitespace-normal break-words text-gray-900 dark:text-gray-100">{{ $task->createdBy->name }}</td>
            <td class="px-6 py-4 text-center text-gray-500 dark:text-gray-400 whitespace-nowrap">{{ $task->created_at->format('d.m.Y H:i') }}</td> @auth <td class="px-6 py-4 text-center">
              <div class="flex items-center gap-3">
